...
unordered_map<string, userIDInfo> getLocalAccounts()
{
    ...
    {
        cout << "No accounts exist please press 2 to create an account" << endl << endl;
        ;
    }
    inFile.close();
    return accounts;
...
void createAccount()
{
    ...
    ofstream outFile;
    outFile.open("managerAccounts.txt", std::ios::app);
    do
    {
...
void addANewPassword(int userID)
{
    ...
    ofstream outFile;
    outFile.open("usersAccounts.txt", ios::app);
...
void findAPassword(int userId)
{
    ...
    {
        string pass = orderedAccounts[userId][application][1];
        //pass = decrypt(pass);
